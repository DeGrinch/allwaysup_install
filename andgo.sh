...
#
# PURPOSE:
#   1) Create system user: allwaysup
#   2) Create folders:
...
#        services/backup/git_auto_push.sh
#        install/initialize_repo.sh
#        install/establish_localrepo_sync_and_git_push_cron.sh
#
# NOTES:
#   - No cloning during install. Only local init + README commit.
#   - Run as root.
#
# INSTALL:
...
# Force HTTP → SSH conversion for origin
...
###############################################################################
# SCRIPT: initialize_repo.sh  (literal values to avoid env issues)
###############################################################################
cat > "$INIT_BIN" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
# Writes cron to ROOT's crontab. Commands run as 'allwaysup'.
###############################################################################
cat > "$CRON_SETUP_BIN" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

# Run hourly as root, but execute scripts as 'allwaysup'
...
# Write to root's crontab. Remove old matching line first, then append cleanly.
{
  crontab -l 2>/dev/null | grep -v "allwaysup_local_sync" || true
  echo "$CRONLINE"
} | crontab -

# Show resulting crontab to confirm
crontab -l
EOF

###############################################################################
# Permissions + kick-off
###############################################################################
